...
class CrossModalMSELoss(nn.Module):
    ...
    def _forward_mse_one_modal(self, recon, target, mask=None):
        loss = (recon - target).abs()
        loss = loss.mean(dim=-1)  # [N, L], mean loss per patch
        if self.use_mask:
    ...
    def forward(self, optical_mask, radar_mask, 
                ...
                optical_target, radar_target,
                output_dict=False, **kwargs):
        optical_recon_radar = optical_recon[:, :, -radar_target.shape[-1]:]
        radar_recon_optical = radar_recon[:, :, :optical_target.shape[-1]]
        optical2radar_mse = self._forward_mse_one_modal(optical_recon_radar, radar_target, optical_mask) * self.lambda_
        radar2optical_mse = self._forward_mse_one_modal(radar_recon_optical, optical_target, radar_mask) * self.lambda_
        if output_dict:
            return dict(optical_radar_mse=optical2radar_mse, radar_optical_mse=radar2optical_mse)
        return optical2radar_mse, radar2optical_mse
