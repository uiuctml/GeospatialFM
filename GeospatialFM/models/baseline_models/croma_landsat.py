...
class PretrainedCROMA_landsat(nn.Module):
    def __init__(self, pretrained_path='CROMA_base.pt', size='base', modality='both', image_resolution=120):
        ...
        self.s2_channels = 20  # particularly accomodate for landsat dataset with 20 channels
        ...
        if modality in ['optical', 'both']:
            ...
            # load weights
            pretrained_dict = torch.load(pretrained_path)['s2_encoder']
            model_dict = self.s2_encoder.state_dict()
            if 'linear_input.weight' in pretrained_dict:
                old_weight = pretrained_dict['linear_input.weight']
                if old_weight.shape[1] != model_dict['linear_input.weight'].shape[1]:  # Only if mismatch
                    pretrained_dict['linear_input.weight'] = F.interpolate(
                        old_weight.unsqueeze(0).unsqueeze(0), 
                        size=(768, 1280), 
                        mode='bilinear',
                        align_corners=False
                    ).squeeze(0).squeeze(0)
            self.s2_encoder.load_state_dict(pretrained_dict, strict=False)
            # self.s2_encoder.load_state_dict(torch.load(pretrained_path)['s2_encoder'], strict=False)
