...
class SpatialSpectralLowRankViTDecoder(PreTrainedModel):
    ...
    def forward(self, x, pos_ids_restore, channel_ids_restore, optical_channel_wv, radar_channel_wv, spatial_resolution, restore_input_dim=False):
        ...
        mask_tokens_mask = torch.isclose(x, self.mask_token.expand_as(x), rtol=1e-5, atol=1e-8)
        ...
        for blk in self.decoder_blocks:
            x = blk(x, spatial_mask=perception_field_mask, channel_mask=mask_tokens_mask_channel, pos_mask=mask_tokens_mask_pos)
...
class SpatialSpectralViTDecoder(PreTrainedModel):
    ...
    def __init__(self, config: SpatialSpectralLowRankViTConfig):
        ...
        self.channel_decoder_blocks = nn.ModuleList([
            Block(
                dim=config.decoder_embed_dim,
                num_heads=config.decoder_num_heads,
                mlp_ratio=config.mlp_ratio,
        ...
        self.pos_decoder_blocks = nn.ModuleList([
            Block(
                dim=config.decoder_embed_dim,
                num_heads=config.decoder_num_heads,
                mlp_ratio=config.mlp_ratio,
    ...
    def forward(self, x, pos_ids_restore, channel_ids_restore, optical_channel_wv, radar_channel_wv, spatial_resolution, restore_input_dim=False):
        ...
        # append mask tokens for channels
        mask_tokens = self.mask_token.expand(B, C - N, L, -1)
        x_ = torch.cat([x[:, 1:], mask_tokens], dim=1)  # B, C, L, D, remove cls token
        x_ = torch.gather(x_, dim=1, index=channel_ids_restore.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, L, D))  # unshuffle, B C L D
        x = torch.cat([x[:, :1], x_], dim=1)  # B C+1 L D, add cls token

        if C == optical_channel_wv.shape[1]: 
            # missing radar channels from encoder
            n_radar_channels = radar_channel_wv.shape[1]
            # extend x to C+n_radar_channels to the end
            mask_tokens = self.mask_token.expand(B, n_radar_channels, L, -1)  # B C L D
            x = torch.cat([x, mask_tokens], dim=1) # B C+1 L D
        elif C == radar_channel_wv.shape[1]:
            # missing optical channels from encoder
            n_optical_channels = optical_channel_wv.shape[1]
            # extend x to C+n_optical_channels to the front behind the cls token
            mask_tokens = self.mask_token.expand(B, n_optical_channels, L, -1)
            x = torch.cat([x[:, :1], mask_tokens, x[:, 1:]], dim=1)  # B C+1 L D
            
        B, C, L, D = x.shape
        x = x.permute(0, 2, 1, 3).reshape(B*L, C, D)
            
        chan_embed = self.pos_chan_embed.get_channel_embed(x[:, 1:, :], channel_ids=channel_wv, cls_token=True).to(x.device, dtype=x.dtype)
        x = x + chan_embed # B*L C D
        
        for blk in self.channel_decoder_blocks:
            x = blk(x)
        
        x = x.reshape(B, L, C, D).permute(0, 2, 1, 3) # B C L D
        
        B, C, L, D = x[:, :, 1:].shape
        HW = pos_ids_restore.shape[1]

        # append mask tokens to sequence
        mask_tokens = self.mask_token.expand(B, C, HW - L, -1)
        x_ = torch.cat([x[:, :, 1:], mask_tokens], dim=2)  # B C+1 HW D, remove cls token
        x_ = torch.gather(x_, dim=2, index=pos_ids_restore.unsqueeze(1).unsqueeze(-1).repeat(1, C, 1, D))  # unshuffle, B C HW D
        x = torch.cat([x[:, :, :1], x_], dim=2)  # B C+1 HW+1 D, add cls token
        
        B, C, HW, D = x.shape
        x = x.reshape(B*C, HW, D)
        ...
        for blk in self.pos_decoder_blocks:
            x = blk(x)
        
        x = x.reshape(B, C, HW, D) # B C HW D
...
class SpatialViTEncoder(PreTrainedModel):
    config_class = SpatialSpectralLowRankViTConfig
    ...
    def forward(self, optical=None, radar=None, optical_channel_wv=None, radar_channel_wv=None, spatial_resolution=10, mask_ratio=0, channel_mask_ratio=0):
        ...
        # Add positional and channel embedding
        pos_chan_embed = self.pos_chan_embed.get_pos_embed(x, spatial_resolution, cls_token=False).to(x.device, dtype=x.dtype)
        x = x + pos_chan_embed
        
        # Apply masks after positional embedding
        x = x.unsqueeze(1) # B 1 HW D
        if self.training:
            x, pos_mask, pos_ids_restore, perception_field_mask = self.random_pos_masking(x, mask_ratio, perception_field_mask=perception_field_mask) # B N L D, B HW, B HW, L L
        else:
            channel_mask = pos_mask = channel_ids_restore = pos_ids_restore = None
        
        x = x.squeeze(1) # B HW D
        B, L, D = x.shape
