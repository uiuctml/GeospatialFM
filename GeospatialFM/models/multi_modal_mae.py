...
class MultiModalMAEViT(nn.Module):
    ...
    def forward_recon(self, optical=None, radar=None, mask_ratio=0.75, slice_patch_tokens=None, channel_mask_ratio=0.5):
        latent, mask, ids_restore, optical_channel_mask = self.encoder.forward_encoder(optical=optical, radar=radar, mask_ratio=mask_ratio, channel_mask_ratio=channel_mask_ratio)
    ...
    def _forward(self, optical, radar, mask_ratio=0.75, channel_mask_ratio=0.5, prefix=''):
        latent, mask, ids_restore, optical_channel_mask = self.encoder.forward_encoder(optical=optical, radar=radar, mask_ratio=mask_ratio, channel_mask_ratio=channel_mask_ratio)
    ...
    def forward(self, optical, radar, mask_ratio=0.75, channel_mask_ratio=0.5, modal=None):
        ...
        if modal is None or modal == 'radar':
            radar_dict = self._forward(None, radar, mask_ratio, channel_mask_ratio, prefix='radar_')
            return_dict.update(radar_dict)
        if modal is None or modal == 'multi':
            multi_dict = self._forward(optical, radar, mask_ratio, channel_mask_ratio, prefix='multi_')
